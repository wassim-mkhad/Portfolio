...
-- Highest borrower
SELECT country_name AS highest_borrower,
	   ROUND(SUM(amount) / 1000000000, 2) AS 'Total amount borrowed in Billions'
FROM ids_data
GROUP BY 1
ORDER BY 2 DESC
...
-- Average borrowing ammount
SELECT 
    ROUND(AVG(`Total amount borrowed in Billions`),
            2) AS 'Average borrowing amount'
FROM
    (SELECT 
        country_name,
            ROUND(SUM(amount) / 1000000000, 2) AS 'Total amount borrowed in Billions'
...
-- Total and percent of total amount borrowed by indicators in China
SELECT 
    indicator_name,
    ROUND(SUM(amount) / 1000000000, 2) AS 'Total in Billion',
    ROUND((SUM(amount) / 1000000000) / (SELECT SUM(amount) / 1000000000
